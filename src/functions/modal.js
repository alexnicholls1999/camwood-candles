...
export function renderProducts() {
  ...
  const renderProductsTables = productsArr
    .map((product) => {
      ...
      return `
            ...
            <table class="products-table">
                ...
                <tbody>
                    <tr class="secondary-row">${table.row1Arr
                      .map((row) => {
                        return `<td>${row}</td>`;
                      })
                      .join(" ")}</tr>
                    <tr>${table.row2Arr
                      ...
                      .join(" ")}</tr>
                    <tr class="secondary-row">${table.row3Arr
                      .map((row) => {
                        return `<td>${row}</td>`;
                      })
                      .join(" ")}</tr>
...
export function toggleModal() {
  ...
  const modalmobileBtn = document.querySelector(
    "[data-ondimensionsmobileclick]"
  );
  ...
  for (let i = 0; i < hyperlinks.length; i++) {
    hyperlinks[i].addEventListener("click", openModal);
  }
